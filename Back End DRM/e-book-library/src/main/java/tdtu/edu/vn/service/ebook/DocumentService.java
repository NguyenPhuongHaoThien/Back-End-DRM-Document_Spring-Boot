...
public class DocumentService {
    ...
    public void saveReadingHistory(String userId, String documentId, int currentPage) {
        ReadingHistory existingHistory = readingHistoryRepository.findTopByUserIdAndDocumentIdOrderByTimestampDesc(userId, documentId);
        if (existingHistory != null) {
            // Nếu đã có bản ghi lịch sử đọc, cập nhật trang hiện tại và thời gian
            existingHistory.setCurrentPage(currentPage);
            existingHistory.setTimestamp(new Date());
            readingHistoryRepository.save(existingHistory);
        } else {
            // Nếu chưa có bản ghi lịch sử đọc, tạo mới
            ReadingHistory newHistory = new ReadingHistory();
            newHistory.setUserId(userId);
            newHistory.setDocumentId(documentId);
            newHistory.setCurrentPage(currentPage);
