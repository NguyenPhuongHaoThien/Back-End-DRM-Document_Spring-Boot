...
public class CartController { // Modify
    ...
    public ResponseEntity<String> addBookToCart(@PathVariable String id, HttpServletRequest request) {
        ...
        if (token != null && token.startsWith("Bearer ")) {
            ...
            // Kiểm tra nếu tài liệu không có DRM
            if (!document.getDrmEnabled()) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Cannot add non-DRM document to cart");
            }

            List<Order> orderList = orderService.findByUserIdAndBookId(user.getId(), document.getId());
            ActivationCode activationCode = acService.findValidCodeWithOrderIdAndBookId(orderList, document.getId());
            if (activationCode != null && activationCode.getStatus() == ActivationCode.ActivationCodeStatus.EXPIRED) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Activation code has expired for this document");
            }
            if (activationCode != null && activationCode.getStatus() == ActivationCode.ActivationCodeStatus.USED) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Activation code is already used");
            }
